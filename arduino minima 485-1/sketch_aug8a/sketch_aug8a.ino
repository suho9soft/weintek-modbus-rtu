...
#define DE_RE 2          // MAX485 DE/RE 제어 핀
#define SENSOR_PIN A0    // 아날로그 센서 입력

// 릴레이 핀 정의 (D4~D12, D9 제외)
const int relayPins[8] = {4,5,6,7,8,10,11,12};
...
void setup() {
  ...
  Serial1.begin(9600); // RS485 연결 (TX→DI, RX→RO)
...
void loop() {
  if (Serial1.available() > 0) {
    uint8_t request[32];
    int len = Serial1.readBytes(request, sizeof(request));
    ...
    if (function == 0x05 && len >= 8) {
      ...
      Serial1.write(request, 8); // 요청 그대로 응답
      Serial1.flush();
    ...
    // -------------------------------
      ...
      digitalWrite(DE_RE, HIGH);
      Serial1.write(response, 5 + byteCount);
      Serial1.flush();
      digitalWrite(DE_RE, LOW);
    }
    ...
    // -------------------------------
      ...
      digitalWrite(DE_RE, HIGH);
      Serial1.write(response, 5 + 2*count);
      Serial1.flush();
      digitalWrite(DE_RE, LOW);
    }
  }
